...
type NSCenterTrainerType =
    (typeof NSCenterTrainerType)[keyof typeof NSCenterTrainerType];

const nonRandomNSCenterTrainerTypes = Object.values(NSCenterTrainerType).filter(
...
function nsCenterTrainerStateToSkewbRendererState(
...
function nsCornerTrainerStateToSkewbRendererState(
    nsCornerTrainerState: NSCornerTrainerState,
) {
    const rotatedCenters = Object.fromEntries(
        Object.values(Color).map((c) => [
            c,
            rotateColor(c, nsCornerTrainerState.rotation),
        ]),
    ) as Record<Color, Color>;
...
type CornerOrientation =
    (typeof CornerOrientation)[keyof typeof CornerOrientation];
...
function nsCornerTrainerStateToCornerOrientation(
    nsCornerTrainerState: NSCornerTrainerState,
) {
    return nsCornerOrientations[
        nsCornerTrainerState.corners.join(
            "|",
        ) as keyof typeof nsCornerOrientations
    ];
}
